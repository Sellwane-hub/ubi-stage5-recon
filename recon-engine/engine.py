...
Supports resume: each stage (ports, signal, http) is checkpointed after it
completes. An interrupted run reloads the checkpoint and skips completed stages,
producing the same result hash as an uninterrupted run (normalization sorts
records by identity before hashing, so assembly order does not matter).
"""
...
def run_discovery(target, scope_path, output_dir, rate=25, resume=True):
    ...
    # append to the ledger on resume so prior requests are preserved
    ledger = RequestLedger(out / "request-ledger.csv")
    req = SafeRequester(guard, ledger)
    errors = []
    notes = []

    # load prior checkpoint state (or a fresh empty state)
    state = checkpoint.load(ckpt_path) if resume else \
        {"completed": [], "records": [], "vhost": None, "signal_port": None}
    records = list(state["records"])
    vhost = state["vhost"]
    signal_port = state["signal_port"]
    resumed_stages = list(state["completed"])
    ...
    # 1. port discovery
    if not checkpoint.is_done(state, "ports"):
        ...
        checkpoint.mark_done(state, "ports")
        checkpoint.save(ckpt_path, state)
    else:
        # recover open ports from the saved records
        open_ports = sorted({r["port"] for r in records})

    # 2. signal discovery
    if not checkpoint.is_done(state, "signal"):
        ...
        state["records"] = records
        state["vhost"] = vhost
        state["signal_port"] = signal_port
        checkpoint.mark_done(state, "signal")
        checkpoint.save(ckpt_path, state)

    # 3. http/vhost discovery (skip the signal port -- it does not speak HTTP)
    if not checkpoint.is_done(state, "http"):
        for port in open_ports:
            if vhost is None:
                break
            if port == signal_port:
        ...
        state["records"] = records
        checkpoint.mark_done(state, "http")
        checkpoint.save(ckpt_path, state)
